...
def main():
    ...
    if len(args.dir) > 0:
        image_names = io.get_image_files(args.dir, "_Cells_3D", imf=imf,
    ...
    for name in image_names:
        ...
        for p in range(3):
            ...
            Ly, Lx = img.shape[1:3]
            imgs = img[np.random.permutation(img.shape[0])[:nimg_per_tif]]
            if args.anisotropy > 1.0 and p > 0:
                imgs = transforms.resize_image(imgs, Ly=int(args.anisotropy * Ly), Lx=Lx, no_channels=no_channels)
            for k, img in enumerate(imgs):
                ...
                img_out = img[ly:ly + args.crop_size, lx:lx + args.crop_size].squeeze()
                io.imsave(os.path.join(dirname, f'train/{name0}_{npm[p]}_{k}.tif'), img_out)

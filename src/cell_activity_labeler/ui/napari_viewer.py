...
from cell_activity_labeler.utils.df_utils import filter_images_by_sample_id_and_z_index
from cell_activity_labeler.utils.io import find_label_from_mcherry_path, ensure_2d, get_images_from_mcherry
from cell_activity_labeler.utils.log import get_viewer_logger
...
class NapariImageViewer:
    ...
    def _set_activity_colors(self, layer):
        ...
        try:
            ...
            _log.debug("layer.color dict assignment failed (%s), trying layer.colors", e)
            try:
                layer.colors = ['black', 'green', 'magenta']
            except Exception as e2:
                _log.debug("layer.colors list assignment failed (%s), falling back to viridis", e2)
                try:
                    layer.colormap = 'viridis'
                    layer.opacity = 0.6
                except Exception as e3:
                    _log.warning("All activity color assignments failed: %s", e3)
    ...
    def close(self):
        ...
        if self.current_viewer is not None:
            try:
                ...
                _log.warning("Error closing napari viewer: %s", e, exc_info=True)
            self.current_viewer = None
    ...
    try:
        ...
        if show_activity:
            ...
            try:
                ...
                _log.debug("Could not set activity colormap: %s", e)
        ...
        _log.error("Error loading image %s: %s", image_name, e, exc_info=True)
...
def show_sample_images_napari(metrics_df, sample_id, root, z_indices=None, max_images=5):
    ...
    for i, image_name in enumerate(sample_images):
        ...
        if viewer is not None:
            try:
                viewer.window._qt_window.move(50 * i, 50 * i)
            except Exception as e:
                _log.debug("Could not reposition napari window: %s", e)
...
def create_napari_3d_from_summary_df(
    ...
    try:
        ...
        _log.error("Error creating napari viewer for sample %s: %s", sample_id, e, exc_info=True)
        ...
        try:
            ...
            if activity_labels is None:
                ...
                try:
                    ...
                    _log.error("Could not create activity labels for %s: %s", image_name, e, exc_info=True)
            ...
            _log.error("Error processing image %s: %s", image_name, e, exc_info=True)

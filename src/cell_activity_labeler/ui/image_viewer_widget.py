...
from cell_activity_labeler.ui.napari_viewer import (
    NapariImageViewer,
...
class ImageViewer:
    ...
    def _safe_load_out(self, timeout: float = 3.0):
        """Capture output to _load_out, waiting for ipykernel's pub_thread first."""
        deadline = time.time() + timeout
        while time.time() < deadline:
            if getattr(sys.stdout, 'pub_thread', True) is not None:
                break
            time.sleep(0.05)
        try:
            with self._load_out:
            ...
            _log.debug("Output widget stream unavailable: %s", e)
        ...
        with self._popup_out:
            ...
            for i, (_, row) in enumerate(sample_rows.iterrows(), start=1):
                ...
                if viewer.current_viewer is not None:
                    try:
                        viewer.current_viewer.window._qt_window.move(50 * (i - 1), 50 * (i - 1))
    ...
    def _open_3d(self):
        ...
        if 'z_index' not in df.columns:
            print("Cannot open 3D view: 'z_index' column is missing. "
                  "Reload CSV or provide a file_handler that adds z_index.")
            return
    ...
    def load_csv(self, csv_path: str | Path) -> bool:
        ...
        try:
            ...
            if 'ID' not in df.columns or 'z-index' not in df.columns:
                file_handler = self.file_handler
                if file_handler is not None:
                    df = add_meta_info(df, file_handler=file_handler)
                else:
                    raise ValueError('CSV is missing "ID" and/or "z_index" columns, and no file_handler was provided to derive them.')
            ...
            _log.error("Error loading CSV: %s", e, exc_info=True)
            with self._safe_load_out():

...
class FeatureToMcherryConfig:
    ...
    feature_csv : str
        ...
        Required — no default, since there is no valid preset pair to fall back to.
    target_csv : str
        Path to an ``src.mcherry_metrics`` instance-metrics CSV. Required, no default.
    id_column : str
        Name of the per-cell id column in the feature CSV (e.g. ``"instance_id"`` for
        the regionprops backend).
    sample_id_column, timepoint_column : str, optional
        Column names in the feature CSV already carrying ``sample_id``/``timepoint``.
        If ``None``, both are derived from the feature CSV's ``image_filename`` column.

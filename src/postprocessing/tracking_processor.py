...
class CellTrackingPipeline:
    ...
    def _get_matching_image_segmentation(self, segmentation_path: Path, image_dir: Path, config: PostprocessingConfig) -> Optional[Path]:
        """
        ...
        Args:
            ...
            config: Postprocessing configuration with image pattern
        """

        # TODO : Implement a file handler to rename files based on patterns

        # Get suffix from config file and replace segmentation patter in segmentation path with image pattern
        seg_pattern = config.mask_pattern.replace("*", "")
        img_pattern = config.image_pattern.replace("*", "")
        # Replace the segmentation pattern in the filename with the image pattern
        image_name = segmentation_path.name.replace(seg_pattern, img_pattern)
    ...
    def process_batch(
        ...
        seg_files = list(mask_dir.glob(self.config.mask_pattern))
        if not seg_files:
        ...
        for seg_file in tqdm(seg_files, desc="Processing files"):
            ...
            image_file = self._get_matching_image_segmentation(seg_file, image_dir, self.config)
            if image_file is None:
                continue

            try:
                result = self.process_single_file(seg_file, image_file, blur_cache_dir=blur_cache_dir, output_manager=output_manager)

...
def create_dummy_tiffs(directory, n=10):
    os.makedirs(directory, exist_ok=True)
    for i in range(n):
        # Use a plate/position naming convention compatible with BF_IF_FileHandler
        # Example: t1_J03_s1_w1_z1.tif and Cells_R10-C3-F0-Z0-T0.tif
        for z in range(5):  # Simulating multiple z-slices
...
class TestSplitSeed(unittest.TestCase):
    def test_split_random_seed_reproducibility(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(set(result1['test_images']), set(result2['test_images']), "Test split is not reproducible with same seed!")
            self.assertEqual(set(result1['train_images']), set(result2['train_images']), "Train split is not reproducible with same seed!")
